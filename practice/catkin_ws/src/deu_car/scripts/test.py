...
class Follower:
  ...
  def image_callback(self, msg):
    ...
    if corners2 is not None:
      ...
      M1 = cv2.moments(l_mask)
    #      if M['m00'] > 0:
    #        self.err = ( center_x2 + center_x ) / 2 - 320
    #        self.twist.linear.x = 0.2
    #        self.twist.angular.z = -float(self.err) / 100
    #        self.cmd_vel_pub.publish(self.twist)

    _, r_line, _ = cv2.findContours(r_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    _, l_line, _ = cv2.findContours(l_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    #   print("WARING !!!!! STOPPPPP !!!")
    #   rospy.sleep(3)
    #   self.time = rospy.Time.now()
    #   self.twist.linear.x = 1.0
    #   self.cmd_vel_pub.publish(self.twist)

    if M1['m00'] > 0 and len(c) < 5:

      if len(r_line) > 0 and len(l_line) > 0:
        # BEGIN FORWARD
        self.err = ( center_x2 + center_x ) / 2 - 320
        print(self.err)
        self.twist.linear.x = 1.0
        self.twist.angular.z = -float(self.err) / 100
        self.cmd_vel_pub.publish(self.twist)
        print ('>>>>>>>>>>>>>>>>>>>>> len(r_line) == ', len(r_line))
        print('<<<<<<<<<<<<<<<<<<<<<<<<<<<< len (l_line) == ', len(l_line))
       # END FORWARD


    elif len(l_line) == 0:
      print('<<<<<<<<<<<<<<<<<< l_line', len(l_line))
      #   self.err = center_x2 / 2 - 320
      #   print("self" , self.twist.angular.z)
      #   self.twist.linear.x = 0.0
      #   self.twist.angular.z = -10
      #   self.cmd_vel_pub.publish(self.twist)
      self.twist.linear.x = 0.0
      self.twist.angular.z = float(200) / 100
      print("TRUN ===================>>>", self.twist.angular.z)
      self.cmd_vel_pub.publish(self.twist)

    elif len(r_line) == 0:
      print("self", self.twist.angular.z)
      self.twist.linear.x = 0.0
      self.twist.angular.z = -float(90) / 100
      self.cmd_vel_pub.publish(self.twist)
    ...
    cv2.imshow("window2", v) # left line
    # cv2.imshow("windows3", r_mask) # right line
    # cv2.imshow("windows4", wmask) #stop line
    cv2.waitKey(3)
